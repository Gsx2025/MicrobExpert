...
# 预测函数
def predict_for_moe(model, passage, question, device, tokenizers, expert_models):
    model.eval()
...
# 训练函数
def train_model(model, train_data_df, tokenizers, expert_models, num_epochs, batch_size, learning_rate):
    ...
    for epoch in range(num_epochs):
        ...
        print(f" Epoch {epoch + 1} Training Loss: {total_train_loss / (len(train_data_df) // batch_size)}")

    return model

# === 主程序逻辑 ===
df = pd.read_csv('/home/stu_6/GSX/data-final/D-M-GSC.csv')
...
for fold, (train_index, test_index) in enumerate(kf.split(df), start=1):
    ...
    moe_model = train_model(moe_model_i, train_data_df, tokenizers, expert_models, 15, 2, 1e-5)
    ...
    for index, row in dev_data_df.iterrows():
        ...
        label = row['RELATION']
        predicted_label = predict_for_moe(moe_model, passage, question, device, tokenizers, expert_models)
